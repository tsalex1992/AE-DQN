...
class ValueBasedLearner(ActorLearner):
    ...
    # returns different epsilon for each actor
    def generate_final_epsilon(self):
        values = [.01, .05, .1, .2]
        return values[self.actor_id % 4]

    ## if the current epsilon is higher than the final anneals it
    ...
    #epsilon is the randomization threshold
    ...
    def bootstrap_min_value(self, state, episode_over):
            if episode_over:
                R = 0
            else:
    ...
    # Returns maximum Q value from state of the target network
...
class NStepQLearner(ValueBasedLearner):
    ...
    def train(self):
        ...
        while (self.global_step.value() < self.max_global_steps):
            ...
            ##### this is the loop from the paper #####
            #TODO add abs(Qupper - Qlower) <epsilon(1-gama)/2
            while not (episode_over
                ...
                ep_t += 1
                # TODO appent to history rewards, states and actions
                ...
                s = new_s
                self.local_step += 1
                ## adding the maximum q reward from the previous state
